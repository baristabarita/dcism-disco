...
const Attire: React.FC = () => {
  ...
  return (
    <section id="attire" className="flex flex-col items-center mt-4">
      <h1 className="text-[7vh] md:text-[10vh] text-center font-light font-bodoni text-[#dadada] glow">Outfit and Attire</h1>
        ...
        {gender === 'men' ? (
            ...
            <div key={index} className="py-1 px-3">
              <Card>
                <CardContent className="bg-black flex items-center justify-center p-4 aspect-auto rounded-lg w-[13em] h-[24em]">
                  <img src={attire} alt={`Men's Attire ${index + 1}`} className="object-cover w-full h-full" />
                </CardContent>
              </Card>
            </div>
          ))
          ...
          womenAttires.map((attire, index) => (
            <div key={index} className="py-1 px-3">
              <Card>
                <CardContent className="bg-black flex items-center justify-center p-4 aspect-auto rounded-lg w-[13em] h-[24em]">
                  <img src={attire} alt={`Women's Attire ${index + 1}`} className="object-cover w-full h-full" />
                </CardContent>
              </Card>
            </div>
          ))
      ...
      <p className="text-[1em] md:text-[1.2em] lg:text-[1.3em] text-center italic font-bitter text-[#dadada] my-4 mx-20 glow">
        Shine your divine radiance in a dazzling outfit that reflects your personality and style. The dress code for the party is semi-formal (or just wear smthn nice idk), with a touch of blueish gray to silver. Be creative and have fun with your look, but remember to keep it classy and comfortable.
      </p>

      <h1 className="font-bold text-[2em] text-[#dadada] font-bitter glow my-4">🪩 IMPORTANT NOTE 🪩</h1>
